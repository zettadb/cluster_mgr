...
public:
  MYSQL_CONN(const char *ip_, int port_, const char *user_, const char *pwd_,
             Shard_node *owner_)
      : connected(false), sqlcmd(SQLCOM_END), port(port_), ip(ip_), user(user_),
        pwd(pwd_), owner(owner_) {
    result = NULL;
    nrows_affected = 0;
    nwarnings = 0;
  }
...
public:
  struct Txn_key {
    ...
    bool operator==(const Txn_key &tk) const {
      return (start_ts == tk.start_ts && local_txnid == tk.local_txnid &&
...
public:
  Shard(uint id_, const std::string &name_, Shard_type type, HAVL_mode mode)
      : thrd_hdlr_assigned(false), cur_master(NULL), shard_type(type),
        ha_mode(mode), id(id_), cluster_id(0), pending_master_node_id(0),
        last_time_check(0), name(name_), m_thrd_hdlr(NULL),
        innodb_page_size(0) {
  ...
  */
  ...
  void add_node(Shard_node *node) {
    ...
    syslog(Logger::INFO,
           "Added shard(%s.%s, %u) node (%s:%d, %u) into protection.",
           cluster_name.c_str(), name.c_str(), id, ip.c_str(), port,
           node->get_id());
  }
  ...
  /*
    Here we assume a node's ID is never changed, its ip/port/user/password can
    be modified. If a node is not found by its ID, we create a new one.
    @param [out] changed : set to true if the node existed and has been updated
    by this call.
  */
...
public:
  ...
  MetadataShard()
      : Shard(METADATA_SHARD_ID, "MetadataShard", METADATA, HA_mgr) {
  ...
  int execute_metadate_opertation(enum_sql_command command,
  ...
  int get_backup_info_from_metadata(std::string &backup_id,
                                    std::string &cluster_name,
