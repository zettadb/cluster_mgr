...
const char *Logger::log_verbosity_options[] = {
    "ERROR", "WARNING", "INFO", "LOG", "DEBUG1", "DEBUG2", "DEBUG3"};
Logger *Logger::m_inst = NULL;

ssize_t Logger::do_syslog(const char *file, const char *func, int lineno,
  ...
  // log timestamp, log verbosity string; only log file/func/lineno if
  // log_verbosity>LOG
  va_list args;
  ...
  if (ret < 0) {
    char errbuf[256];
    /*
            If we can't write into the log file we'd better write to stderr
       instead.
    */
    dprintf(2, "Failed to write to error log file, error is %d, %s.", errno,
  ...
  if (cur_bytes > get_max_log_file_size() &&
      doing_rotation.compare_exchange_strong(is_rotating, true) && !is_rotating)
    rotate();
  return slen;
}
...
int Logger::generate_log_fn(const std::string &lfp, std::string &fn) {
  ...
  int len = snprintf(buf, sizeof(buf), "-%d-%.2d-%.2d", tmnow.tm_year + 1900,
...
int Logger::rotate() {
  ...
  fd = open(fn.c_str(), O_APPEND | O_NOATIME | O_CREAT | O_CLOEXEC | O_RDWR,
            0644);
  if (fd < 0) {
    char buf[256];
    dprintf(log_fd > 0 ? log_fd : 1,
            "Failed to open file %s for rotation, got error (%d: %s)",
            fn.c_str(), errno, strerror_r(errno, buf, sizeof(buf)));
    ret = -1;
