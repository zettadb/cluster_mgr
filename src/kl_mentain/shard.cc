...
#define IS_MYSQL_CLIENT_ERROR(err)                                             \
  (((err) >= CR_MIN_ERROR && (err) <= CR_MAX_ERROR) ||                         \
   ((err) >= CER_MIN_ERROR && (err) <= CER_MAX_ERROR))

static void convert_preps2ti(Shard *ps, const Shard::Prep_recvrd_txns_t &preps,
                             MetadataShard::cluster_txninfo &cti,
                             bool is_meta_shard);
static void process_prep_txns(
    const Shard::Txn_decision &txn_dsn, MetadataShard::txn_info &ti,
...
int MYSQL_CONN::connect() {
  ...
  MYSQL *ret = mysql_real_connect(
      &conn, ip.c_str(), user.c_str(), pwd.c_str(),
      owner->owner->get_id() == MetadataShard::METADATA_SHARD_ID
          ? KUNLUN_METADATA_DBNAME
          : NULL,
      port, NULL,
  ...
  connected =
      true; // check_mysql_instance_status() needs this set to true here.
...
int MYSQL_CONN::verify_version() {
  ...
  while ((row = mysql_fetch_row(result))) {
    ...
    if (strcasestr(verstr, "kunlun-storage"))
      ...
      syslog(Logger::ERROR,
             "Unsupported mysql version %s, must use kunlun-storage-8.0.x",
             verstr);
      ret = -2;
    }
    break;
  ...
  if (ret == -1)
    syslog(Logger::ERROR,
           "Version information unknown, can't handle mysql instance(%s:%p).",
...
int MYSQL_CONN::handle_mysql_error(const char *stmt_ptr, size_t stmt_len) {
  ...
  syslog(Logger::ERROR,
         "Got error executing '%s' from MySQL server (%s:%d) of shard (%s.%s, "
         "%u) node(%u): {%u: %s}%s.",
         stmt_ptr ? stmt_ptr : "<none>", ip.c_str(), port,
         owner->owner->get_cluster_name().c_str(),
         owner->owner->get_name().c_str(), owner->owner->get_id(),
         owner->get_id(), ret, errmsg_buf, extra_msg);

  return ret;
}

bool Shard_node::update_conn_params(const char *ip_, int port_,
  ...
  if (changed && mysql_conn.connected) {
    syslog(Logger::INFO,
           "Connection parameters for shard (%s.%s %u) node (%u, %s:%d) "
           "changed to (%s:%d, %s, ***), reconnected with new params",
           owner->get_cluster_name().c_str(), owner->get_name().c_str(),
           owner->get_id(), id, old_ip.c_str(), old_port, ip_, port_, user_);
    // mysql_conn.close_conn(); can't do it here because this node may have a
    // valid 'result' being iterated.
    mysql_conn.connect();
  }
  return changed;
}
...
bool MYSQL_CONN::handle_mysql_result() {
  ...
  if (sqlcmd == SQLCOM_SELECT) {
    ...
    if (mysql_res) {
      if (result) {
        syslog(Logger::ERROR, "MySQL result not consumed/freed before sending "
                              "a SELECT statement.");
        return true;
      } else {
    ...
    do {
      ...
      nrows_affected += n;
      // TODO: handle RETURNING result later, and below Assert will need be
      // removed.
      Assert(mysql_field_count(&conn) == 0);
       ...
       * looping) Note that mariadb's client library doesn't return -1 to
       * indicate no more results, we have to call mysql_more_results() to see
...
bool MYSQL_CONN::send_stmt(enum_sql_command sqlcom_, const char *stmt,
  ...
  if (!connected) {
    syslog(Logger::ERROR,
           "Connection to shard (%s.%s, %u) node(%u, %s:%d) broken.",
           owner->owner->get_cluster_name().c_str(),
           owner->owner->get_name().c_str(), owner->owner->get_id(), owner->id,
           ip.c_str(), port);
    return true;
  }
...
*/
bool Shard_node::send_stmt(enum_sql_command sqlcom_, const char *stmt,
                           size_t len, int nretries) {
  bool ret = true;
...
bool Shard_node::send_stmt(enum_sql_command sqlcom_, const std::string &stmt,
                           int nretries) {
  return send_stmt(sqlcom_, stmt.c_str(), stmt.length(), nretries);
}

int Shard_node::connect() { return mysql_conn.connect(); }
...
int Shard_node::start_mgr(Group_member_status curstat, bool as_master) {
  ...
  if (as_master)
    extra_info = " as primary";

  static const char stmt1[] = "stop group_replication; start group_replication";
  static const char stmt2[] = "start group_replication";
  static const char stmt3[] =
      "stop group_replication; SET GLOBAL "
      "group_replication_bootstrap_group=ON; START GROUP_REPLICATION; SET "
      "GLOBAL group_replication_bootstrap_group=OFF";
  static const char stmt4[] =
      "SET GLOBAL group_replication_bootstrap_group=ON; START "
      "GROUP_REPLICATION; SET GLOBAL group_replication_bootstrap_group=OFF";
  ...
  */
  ...
  if (ret) {
    syslog(Logger::ERROR,
           "Failed to join shard (%s.%s, %u) node (%u, %s:%d) of status %s "
           "back to the MGR cluster%s.",
           owner->get_cluster_name().c_str(), owner->get_name().c_str(),
  ...
  syslog(Logger::INFO,
         "Added shard (%s.%s, %u) node (%u, %s:%d) of status %s back to the "
         "MGR cluster%s.",
         owner->get_cluster_name().c_str(), owner->get_name().c_str(),
         owner->get_id(), this->id, mysql_conn.ip.c_str(), mysql_conn.port,
         Group_member_status_strs[curstat - 1], extra_info);
...
bool Shard_node::fetch_mgr_progress() {
  bool ret = send_stmt(
      SQLCOM_SELECT,
      CONST_STR_PTR_LEN("select interval_end from mysql.gtid_executed where "
                        "source_uuid=@@group_replication_group_name"),
      stmt_retries);
  if (ret)
    return ret;
...
int Shard_node::get_mgr_master_ip_port(std::string &ip, int &port) {
  ...
  if (nrows != 1) {
    syslog(Logger::WARNING,
           "Wrong NO.(%lu) of primary nodes found in shard (%s.%s, %u) "
           "node(%u, %s:%d), it could be joining the MGR cluster.",
...
/*
  ...
  -5: MGR cluster doesn't have this node itself, it returned a bunch of other
  nodes. -6: invalid row field values in returned results. positive:
  Group_member_status enums;
*/
int Shard_node::check_mgr_state() {
  const char *the_stmt = NULL;
  int ret = send_stmt(
      SQLCOM_SELECT,
      the_stmt = CONST_STR_PTR_LEN(
  ...
  while ((row = mysql_fetch_row(result))) {
    if (row[0][0] == '\0' && row[1] == NULL && row[3][0] == '\0' &&
        nrows == 1) {
    ...
    if (row[1] == NULL) {
      syslog(Logger::ERROR,
             "Invalid SQL query (%s) result(%lu rows) returned from "
             "shard(%s.%s, %u) node(%s:%d, %u): (%s, NULL, %s, %s)",
             the_stmt, nrows, owner->get_cluster_name().c_str(),
             owner->get_name().c_str(), owner->get_id(), mysql_conn.ip.c_str(),
             mysql_conn.port, this->id, row[0], row[2], row[3]);
      ret = -6;
      goto end;
    }
    ...
    Assert(row[0][0] && row[1] &&
           row[2][0]); // && row[3][0]); could be "" when state is OFFLINE.
  ...
  if (nrows == 0) {
    syslog(Logger::ERROR,
           "Group replication not initialized in shard (%s.%s, %u) node (%u, "
           "%s:%d), this node is invalid as a shard node for Kunlun DDC.",
           owner->get_cluster_name().c_str(), owner->get_name().c_str(),
           owner->get_id(), this->id, mysql_conn.ip.c_str(), mysql_conn.port);
    ret = -2;
  } else if (n_myrows == 0) {
    ...
    syslog(Logger::ERROR,
           "Invalid group replication cluster status in shard (%s.%s, %u) node "
           "(%u, %s:%d): no valid rows found representing this node itself.",
           owner->get_cluster_name().c_str(), owner->get_name().c_str(),
           owner->get_id(), this->id, mysql_conn.ip.c_str(), mysql_conn.port);
    ret = -5;
  } else if (n_myrows > 1) {
    syslog(Logger::ERROR,
           "Invalid group replication cluster status in shard (%s.%s, %u) node "
           "(%u, %s:%d): %lu rows found for this node",
           owner->get_cluster_name().c_str(), owner->get_name().c_str(),
           owner->get_id(), this->id, mysql_conn.ip.c_str(), mysql_conn.port,
           n_myrows);
    ret = -3;
    ...
    syslog(Logger::ERROR,
           "Invalid group replication node status in shard (%s.%s, %u) node "
           "(%u, %s:%d): unrecognized node status %s",
           owner->get_cluster_name().c_str(), owner->get_name().c_str(),
           owner->get_id(), this->id, mysql_conn.ip.c_str(), mysql_conn.port,
           node_stat.c_str());
    ret = -4;
...
const char *Shard_node::Group_member_status_strs[] = {
    "ONLINE", "OFFLINE", "RECOVERING", "ERROR", "UNREACHABLE", "INVALID"};
...
int Shard::check_mgr_cluster() {
  ...
  for (auto &i : nodes) {
    ...
    // if stat == -1 or MEMBER_UNREACHABLE, node i can't be connected for now,
    // it probably will be started up and come back again later, so leave it
    // alone for now. for MEMBER_IN_RECOVERY, node i will very soon be usable
    // with no further action needed.
    if (stat == Shard_node::MEMBER_ERROR ||
        stat == Shard_node::MEMBER_OFFLINE) {
    ...
    // node and 2 ERROR/OFFLINE nodes, we still need to bring the cluster up
    // entirely.
  ...
  if (likely(nodes_down == 0))
    return 0; // most common case, we trust MGR will not brainsplit.
  ...
  if (nodes_down < nodes.size()) {
    ...
    /*
      ...
      Doing so requires simple majority nodes to be connectable/reachable,
      otherwise we might lose some changes if the unreachable nodes happen to
      have changes not in the connectable/reachable nodes.
    */
    if (reachables > nodes.size() / 2) {
      ...
      for (auto itr = down_reachables.begin(); itr != down_reachables.end();
           ++itr) {
        if (Thread_manager::do_exit)
          break;
      ...
      Assert((max_sn != NULL && max_pos > 0 &&
              max_stat != Shard_node::MEMBER_END) ||
             (max_sn == NULL && max_pos == 0 &&
              max_stat == Shard_node::MEMBER_END));
      if (!max_sn)
        goto out1; // it's likely that MGR isn't activated in the cluster.

      max_sn->get_ip_port(top_ip, top_port);
      // max_sn found, start it as master, start the rest as slaves.
      syslog(Logger::INFO,
             "Found shard (%s.%s, %u) node(%u, %s:%d) has max gtid position: "
             "%llu and will be chosen as primary node.",
             get_cluster_name().c_str(), get_name().c_str(), get_id(),
             max_sn->get_id(), top_ip.c_str(), top_port, max_pos);
      set_master(max_sn);
    got_new_master:
      /*
      ...
      */
      if (max_sn->start_mgr(max_stat == Shard_node::MEMBER_END
                                ? Shard_node::MEMBER_ERROR
                                : max_stat,
                            true)) {
        Assert(this->pending_master_node_id == 0 ||
               this->pending_master_node_id == max_sn->get_id());

        // 3093: The START GROUP_REPLICATION command failed since the group is
        // already running. this error code isn't in mariadb's client header, so
        // no macro for it.
      ...
      // if we end up started no more than half of all nodes, we still get an
      // error
      if (reachables <= nodes.size() / 2)
        goto out1;
  ...
  */
  ...
  for (auto &i : nodes) {
    ...
    if (unreachables.find(i) == unreachables.end()) {
      ...
      while ((retx = i->get_mgr_master_ip_port(new_master_ip,
                                               new_master_port)) != 0 &&
      ...
      if (master_port == 0) {
      ...
      } else if (new_master_port != 0 && (master_port != new_master_port ||
                                          master_ip != new_master_ip)) {
        /*
        ...
        */
        ...
        syslog(Logger::ERROR,
               "Found in shard (%s.%s, %u) node(%u, %s:%d) a different primary "
               "node %s:%d than the one already found: %s:%d. A primary switch "
               "might be going on.",
               get_cluster_name().c_str(), get_name().c_str(), get_id(),
               i->get_id(), iip.c_str(), iport, new_master_ip.c_str(),
  ...
  if (!master_port) {
    syslog(Logger::ERROR,
           "Found in shard (%s.%s, %u) no MGR primary node when we've attemted "
           "to join all reachable nodes to the MGR cluster.",
           get_cluster_name().c_str(), get_name().c_str(), get_id());
    return -3;
  }
  if (ndone < nodes.size() / 2) {
    syslog(Logger::ERROR,
           "Found in shard (%s.%s, %u) of %u nodes the primary node confirmed "
           "to be %s:%d by only %d nodes, not majority quorum.",
           get_cluster_name().c_str(), get_name().c_str(), get_id(),
  ...
  if (!found_master) {
    syslog(Logger::ERROR,
           "Latest primary node (%s:%d) in shard (%s.%s, %u) not registered "
           "into cluster manager yet",
  ...
  syslog(Logger::INFO,
         "Found current primary node (%s:%d, %u) in shard (%s.%s, %u) of %u "
         "nodes confirmed by %u nodes.",
         master_ip.c_str(), master_port, the_master->get_id(),
         get_cluster_name().c_str(), get_name().c_str(), get_id(), nodes.size(),
         ndone);
  return 0;
}
...
int System::process_recovered_prepared() {
  ...
  MetadataShard::cluster_txninfo meta_tki(meta_shard.get_cluster_id(),
  ...
  for (auto &sd : all_shards) {
    ...
    if (itr == cluster_txns.end()) {
      MetadataShard::cluster_txninfo cti(sd->get_cluster_id(),
                                         sd->get_cluster_name());
...
*/
int MetadataShard::compute_txn_decisions(
    std::map<uint, cluster_txninfo> &cluster_txns) {
  char qstr_buf[256];
    ...
    */
...

    {
      ...
      int slen = snprintf(qstr_buf, sizeof(qstr_buf),
                          "select txn_id, comp_node_id, next_txn_cmd, "
                          "unix_timestamp(prepare_ts) from commit_log_%s where "
                          "txn_id >= %lu and txn_id <= %lu order by txn_id",
...
int Shard::end_recovered_prepared_txns() {
  ...
  for (auto &td : txn_dcsns) {
    ...
    if (cur_master->send_stmt(
            (td.decision == COMMIT ? SQLCOM_XA_COMMIT : SQLCOM_XA_ROLLBACK),
            txnid_buf, slen, stmt_retries)) {
...
static void convert_preps2ti(Shard *ps, const Shard::Prep_recvrd_txns_t &preps,
  ...
  for (auto &tk : preps) {
    ...
    // there is no commit log for DDL txns, don't enlarge the scanned range in
    // vain.
    if (is_meta_shard)
      continue;

    if (cti.min_start_ts > start_ts)
      cti.min_start_ts = start_ts;
    if (cti.max_start_ts < start_ts)
      cti.max_start_ts = start_ts;
  }
}
...
int Shard::get_xa_prepared() {
...

  {
    ...
    int ret = cur_master->send_stmt(
        SQLCOM_SELECT,
        CONST_STR_PTR_LEN("select trx_xid from information_schema.innodb_trx "
                          "where trx_xa_type='external_recvrd'"),
        stmt_retries);
    if (ret)
      return ret;
    MYSQL_RES *result = cur_master->get_result();
    ...
    while ((row = mysql_fetch_row(result))) {
      ...
      if (sp1)
        sp2 = strchr(sp1 + 1, '-');
      if (sp2)
        sp3 = strchr(sp2 + 1, '-'); // sp3 should be NULL

      if (!(row[0][0] == '\'' && row[0][lengths[0] - 1] == '\'') ||
          sp1 == NULL || sp2 == NULL || sp3) {
        syslog(
            Logger::WARNING,
            "Got XA transaction ID %s from shard (%s.%s, %u) primary node(%u, "
            "%s:%d), not under Kunlun DRDBMS control, and it's skipped.",
  ...
  syslog(Logger::LOG,
         "Got %lu prepared txns in shard (%s.%s, %u) primary node(%u, %s:%d).",
         txns.size(), this->cluster_name.c_str(), this->name.c_str(), this->id,
         cur_master->get_id(), mip.c_str(), mport);
end:
  return 0;
}
...
uint Shard::get_innodb_page_size() {
  if (innodb_page_size == 0) {
    ...
    int ret = cur_master->send_stmt(
        SQLCOM_SELECT,
        CONST_STR_PTR_LEN("show variables like 'innodb_page_size'"),
        stmt_retries);
...
int MetadataShard::refresh_shards(std::vector<KunlunCluster *> &kl_clusters) {
  ...
  int ret = cur_master->send_stmt(
      SQLCOM_SELECT,
      CONST_STR_PTR_LEN(
          "select t1.id as shard_id, t1.name, t2.id, hostaddr, port, "
          "user_name, passwd, t3.name, t3.id as cluster_id, t3.ha_mode from "
          "shards t1, shard_nodes t2, db_clusters t3 where t2.shard_id = t1.id "
          "and t3.id=t1.db_cluster_id order by t1.id"),
      stmt_retries);
  if (ret)
    return ret;
  MYSQL_RES *result = cur_master->get_result();
  ...
  while ((row = mysql_fetch_row(result))) {
    ...
    if (!pcluster) {
      ...
      syslog(Logger::INFO, "Added KunlunCluster(%s.%u) into protection.",
             row[7], cluster_id);
    }
    ...
    */
    ...
    if (changed)
      pshard->get_node_by_id(nodeid)->close_conn();
  ...
  // Remove shard nodes that are no longer in the shard, they are all left in
  // sdns.
  for (auto &i : sdns) {
    ...
    syslog(Logger::INFO,
           "Removed shard(%s.%s, %u) node (%s:%d, %u) from protection since "
           "it's not in cluster registration anymore.",
           pshard->get_cluster_name().c_str(), pshard->get_name().c_str(),
...
*/
int MetadataShard::refresh_computers(
  ...
  for (auto &cluster : kl_clusters) {
    str_sql = "select id,name,hostaddr,port,user_name,passwd from comp_nodes "
              "where db_cluster_id=" +
              std::to_string(cluster->get_id());

    // syslog(Logger::INFO, "refresh_computers str_sql = %s", str_sql.c_str());
    ret = cur_master->send_stmt(SQLCOM_SELECT, str_sql.c_str(),
                                str_sql.length(), stmt_retries);
    if (ret)
      return ret;
    ...
    while ((row = mysql_fetch_row(result))) {
      ...
      if (!pcomputer) {
        pcomputer = new Computer_node(compid, cluster->get_id(), port, row[1],
                                      row[2], row[4], row[5]);
        cluster->computer_nodes.push_back(pcomputer);
        syslog(Logger::INFO, "Added Computer(%s, %u, %s) into protection.",
               cluster->get_name().c_str(), pcomputer->id,
               pcomputer->name.c_str());

        alterant_node_ip.insert(row[2]);
      } else {
        if (pcomputer->refresh_node_configs(port, row[1], row[2], row[4],
                                            row[5]))
          alterant_node_ip.insert(row[2]);
      }
    ...
    // Remove computer nodes that are no longer in the computer_nodes, they are
    // all left in sdns.
    for (auto &i : sdns) {
      std::string ip;
      int port;
      ...
      for (auto it = cluster->computer_nodes.begin();
           it != cluster->computer_nodes.end();) {
        if ((*it)->id == i.first) {
          delete *it;
...
int MetadataShard::check_port_used(std::string &ip, int port) {
  ...
  std::string str_sql =
      "select hostaddr,port from meta_db_nodes where hostaddr=\"" + ip +
      "\" and port=" + std::to_string(port);
  ret = cur_master->send_stmt(SQLCOM_SELECT, str_sql.c_str(), str_sql.length(),
                              stmt_retries);
  ...
  str_sql = "select hostaddr,port from shard_nodes where hostaddr=\"" + ip +
            "\" and port=" + std::to_string(port);
  ret = cur_master->send_stmt(SQLCOM_SELECT, str_sql.c_str(), str_sql.length(),
                              stmt_retries);
  ...
  str_sql = "select hostaddr,port from comp_nodes where hostaddr=\"" + ip +
            "\" and port=" + std::to_string(port);
  ret = cur_master->send_stmt(SQLCOM_SELECT, str_sql.c_str(), str_sql.length(),
                              stmt_retries);
...
int MetadataShard::get_comp_nodes_id_seq(int &comps_id) {
  ...
  int ret = cur_master->send_stmt(
      SQLCOM_SELECT, CONST_STR_PTR_LEN("select max(id) from comp_nodes_id_seq"),
      stmt_retries);
  if (ret == 0) {
    MYSQL_RES *result = cur_master->get_result();
    MYSQL_ROW row;
...
/*
  ...
  @retval 0 succeed;
                  1 fail;
*/
int MetadataShard::execute_metadate_opertation(enum_sql_command command,
  ...
  int ret = cur_master->send_stmt(command, str_sql.c_str(), str_sql.length(),
                                  stmt_retries);
...
/*
  ...
  @retval 0 succeed;
                  1 fail;
*/
int MetadataShard::delete_cluster_from_metadata(
  ...
  // get cluster_id
  std::string str_sql =
      "select id from db_clusters where name='" + cluster_name + "'";
  ret = cur_master->send_stmt(SQLCOM_SELECT, str_sql.c_str(), str_sql.length(),
                              stmt_retries);
  if (ret == 0) {
    MYSQL_RES *result = cur_master->get_result();
  ...
  // get comp_id
  str_sql = "select id from comp_nodes where db_cluster_id=" +
  ...
  // remove comp_nodes
  str_sql = "delete from comp_nodes where db_cluster_id=" +
            std::to_string(cluster_id);
  ret = cur_master->send_stmt(SQLCOM_DELETE, str_sql.c_str(), str_sql.length(),
                              stmt_retries);
  if (ret != 0)
    return ret;
  ...
  for (auto &comp_id : vec_comp_id) {
    str_sql =
        "delete from comp_nodes_id_seq where id=" + std::to_string(comp_id);
    ret = cur_master->send_stmt(SQLCOM_DELETE, str_sql.c_str(),
                                str_sql.length(), stmt_retries);
    if (ret != 0)
      return ret;
  }
  ...
  // remove shards
  str_sql =
      "delete from shards where db_cluster_id=" + std::to_string(cluster_id);
  ret = cur_master->send_stmt(SQLCOM_DELETE, str_sql.c_str(), str_sql.length(),
                              stmt_retries);
  if (ret != 0)
    return ret;
  ...
  // remove shard_nodes
  str_sql = "delete from shard_nodes where db_cluster_id=" +
            std::to_string(cluster_id);
  ret = cur_master->send_stmt(SQLCOM_DELETE, str_sql.c_str(), str_sql.length(),
                              stmt_retries);
  if (ret != 0)
  ...
  ret = cur_master->send_stmt(SQLCOM_DROP_TABLE, str_sql.c_str(),
                              str_sql.length(), stmt_retries);
  if (ret != 0)
    return ret;
...
/*
  ...
  @retval 0 succeed;
                  1 fail;
*/
int MetadataShard::get_server_nodes_from_metadata(
  ...
  std::string str_sql =
      "select hostaddr,datadir,logdir,wal_log_dir,comp_datadir from "
      "server_nodes";
  int ret = cur_master->send_stmt(SQLCOM_SELECT, str_sql.c_str(),
                                  str_sql.length(), stmt_retries);
  if (ret == 0) {
    MYSQL_RES *result = cur_master->get_result();
    MYSQL_ROW row;
...
/*
  ...
  @retval 0 succeed;
                  1 fail;
*/
int MetadataShard::get_backup_info_from_metadata(std::string &backup_id,
  ...
  std::string str_sql =
      "select cluster_name,shards,when_created from cluster_backups where id=" +
      backup_id;
  int ret = cur_master->send_stmt(SQLCOM_SELECT, str_sql.c_str(),
                                  str_sql.length(), stmt_retries);
  if (ret == 0) {
...
/*
  ...
  This method is can be called repeatedly to refresh metashard nodes
  periodically.
  @param is_master whether 'sn' is current master node,
    if so 'master_ip' and 'master_port' are 0;
        otherwise, 'master_ip' and 'master_port' are the current master's conn
  info.
*/
int MetadataShard::fetch_meta_shard_nodes(Shard_node *sn, bool is_master,
  ...
  int ret = sn->send_stmt(
      SQLCOM_SELECT,
      CONST_STR_PTR_LEN(
          "select id, hostaddr, port, user_name, passwd from meta_db_nodes"),
      stmt_retries);
  if (ret)
    return ret;
    ...
    if (!is_master && master_ip && strcmp(master_ip, row[1]) == 0 &&
      ...
      */
      syslog(Logger::INFO,
             ...
             get_cluster_name().c_str(), get_name().c_str(), get_id(),
             master_ip, master_port, node->get_id());
    }
  }
  ...
  */
  for (auto &i : snodes) {
    ...
    syslog(Logger::INFO,
           "Removed shard(%s.%s, %u) node(%s:%d, %u) which no longer belong to "
           "the meta shard.",
           ps->get_cluster_name().c_str(), ps->get_name().c_str(), ps->get_id(),
           snip.c_str(), snport, pn->get_id());
...
bool Shard::set_thread_handler(Thread *h, bool force) {
  ...
  if (!h || (thrd_hdlr_assigned.compare_exchange_strong(hdlr_assigned, true) &&
             !hdlr_assigned)) {
    Scopped_mutex sm(mtx);
    if ((h && !m_thrd_hdlr &&
         (
#ifdef ENABLE_DEBUG
             /*
               so that tests could run faster. Can't enable in release build
               otherwise it could be exploited to drain system resources and
               causes a DoS attack.
             */
             force ||
#endif
